...
class NewFusion(nn.Module):
    def __init__(self, num_feat=64, num_frame=14, center_frame_idx=0):
        ...
        m_res_block1 = [common.ResBlock(conv, num_feat, kernel_size=3) for _ in range(n_resblocks)]

        m_res_block2 = [common.ResBlock(conv, num_feat * 2, kernel_size=3) for _ in range(n_resblocks)]

        m_res_block3 = [common.ResBlock(conv, num_feat * 4, kernel_size=3) for _ in range(n_resblocks)]

        m_res_block4 = [common.ResBlock(conv, num_feat * 4, kernel_size=3) for _ in range(n_resblocks)]

        m_res_block5 = [common.ResBlock(conv, num_feat * 2, kernel_size=3) for _ in range(n_resblocks)]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.input_proj = InputProj(
            in_channel=embed_dim, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU
        )
        ...
        self.output_proj_HG1_0 = OutputProj_HWC(
            in_channel=2 * embed_dim, out_channel=embed_dim, kernel_size=3, stride=1
        )

        self.output_proj_HG2_0 = OutputProj_HWC(
            in_channel=8 * embed_dim, out_channel=4 * embed_dim, kernel_size=3, stride=1
        )
        self.output_proj_HG2_1 = OutputProj_HWC(
            in_channel=4 * embed_dim, out_channel=2 * embed_dim, kernel_size=3, stride=1
        )
        ...
        m_body = [common.ResBlock(conv, embed_dim, kernel_size=3) for _ in range(n_resblocks)]

        self.fusion = NewFusion(num_feat=embed_dim, num_frame=self.num_frames, center_frame_idx=0)

        m_tail = [common.Upsampler(conv, scale, embed_dim, act=False), conv(embed_dim, in_chans, kernel_size=3)]
        ...
        ###### HG Block1 ######
        # Encoder
        HG1_res_enc_0 = [common.ResBlock(conv, embed_dim, kernel_size=3) for _ in range(n_resblocks)]
        self.HG1_res_enc0 = nn.Sequential(*HG1_res_enc_0)
        self.HG1_encoderlayer_0 = BasicBaseModelLayer(
            dim=embed_dim,
        ...
        HG1_res_enc_1 = [common.ResBlock(conv, embed_dim * 2, kernel_size=3) for _ in range(n_resblocks)]
        self.HG1_res_enc1 = nn.Sequential(*HG1_res_enc_1)
        self.HG1_encoderlayer_1 = BasicBaseModelLayer(
            dim=embed_dim * 2,
        ...
        # Bottleneck
        self.conv_HG1 = BasicBaseModelLayer(
            dim=embed_dim * 4,
        ...
        HG1_res_dec_0 = [common.ResBlock(conv, embed_dim * 4, kernel_size=3) for _ in range(n_resblocks)]
        self.HG1_res_dec0 = nn.Sequential(*HG1_res_dec_0)
        self.HG1_decoderlayer_0 = BasicBaseModelLayer(
            dim=embed_dim * 4,
            output_dim=embed_dim * 4,
            input_resolution=(img_size // 2, img_size // 2),
            depth=depths[5],
        ...
        HG1_res_dec_1 = [common.ResBlock(conv, embed_dim * 2, kernel_size=3) for _ in range(n_resblocks)]
        self.HG1_res_dec1 = nn.Sequential(*HG1_res_dec_1)
        self.HG1_decoderlayer_1 = BasicBaseModelLayer(
            dim=embed_dim * 2,
            output_dim=embed_dim * 2,
            input_resolution=(img_size, img_size),
            depth=depths[6],
        ...
        ###### HG Block2 ######
        # Encoder
        HG2_res_enc_0 = [common.ResBlock(conv, embed_dim, kernel_size=3) for _ in range(n_resblocks)]
        self.HG2_res_enc0 = nn.Sequential(*HG2_res_enc_0)
        self.HG2_encoderlayer_0 = BasicBaseModelLayer(
            dim=embed_dim,
            output_dim=embed_dim,
            input_resolution=(img_size, img_size),
            depth=depths[0],
        ...
        HG2_res_enc_1 = [common.ResBlock(conv, embed_dim * 2, kernel_size=3) for _ in range(n_resblocks)]
        self.HG2_res_enc1 = nn.Sequential(*HG2_res_enc_1)
        self.HG2_encoderlayer_1 = BasicBaseModelLayer(
            dim=embed_dim * 2,
            output_dim=embed_dim * 2,
            input_resolution=(img_size // 2, img_size // 2),
            depth=depths[1],
        ...
        # Bottleneck
        self.conv_HG2 = BasicBaseModelLayer(
            dim=embed_dim * 4,
            output_dim=embed_dim * 4,
            input_resolution=(img_size // (2**2), img_size // (2**2)),
            depth=depths[4],
        ...
        HG2_res_dec_0 = [common.ResBlock(conv, embed_dim * 4, kernel_size=3) for _ in range(n_resblocks)]
        self.HG2_res_dec0 = nn.Sequential(*HG2_res_dec_0)
        self.HG2_decoderlayer_0 = BasicBaseModelLayer(
            dim=embed_dim * 4,
            output_dim=embed_dim * 4,
            input_resolution=(img_size // 2, img_size // 2),
            depth=depths[5],
        ...
        HG2_res_dec_1 = [common.ResBlock(conv, embed_dim * 2, kernel_size=3) for _ in range(n_resblocks)]
        self.HG2_res_dec1 = nn.Sequential(*HG2_res_dec_1)
        self.HG2_decoderlayer_1 = BasicBaseModelLayer(
            dim=embed_dim * 2,
            output_dim=embed_dim * 2,
            input_resolution=(img_size, img_size),
            depth=depths[6],
