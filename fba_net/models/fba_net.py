...
from ..blocks.federated_affinity_fusion import FAFBlock
from ..blocks.residual import ResBlock
from ..blocks.upsampler import UpsamplerBlock
from ..layers.conv2d import Conv2dLayer
from ..layers.downsample import DownsampleLayer
from ..layers.input_projection import InputProjLayer
from ..layers.output_projection import OutputProjLayer
from ..layers.output_projection_hwc import OutputProjHWCLayer
from ..layers.upsample import UpsampleLayer


class FBANetModel(eqx.Module, strict=True, frozen=True, kw_only=True):
    # Input attributes
    ...
    def __post_init__(self, key: jrandom.KeyArray) -> None:
        object.__setattr__(self, "pos_drop", nn.Dropout(self.drop_rate))
        # Input/Output
        keys = list(jrandom.split(key, 10))
        object.__setattr__(
            self,
            "input_proj",
            InputProjLayer(in_channels=self.in_channels, out_channels=self.embed_dim, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "output_proj",
            OutputProjHWCLayer(in_channels=2 * self.embed_dim, out_channels=self.embed_dim, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "output_proj_2",
            OutputProjLayer(in_channels=2 * self.embed_dim, out_channels=self.embed_dim, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "output_proj_HG2_0",
            OutputProjHWCLayer(in_channels=8 * self.embed_dim, out_channels=4 * self.embed_dim, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "output_proj_HG2_1",
            OutputProjHWCLayer(in_channels=4 * self.embed_dim, out_channels=2 * self.embed_dim, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "fusion",
        ...
        object.__setattr__(
            self, "head", Conv2dLayer(in_channels=self.in_channels, out_channels=self.embed_dim, key=keys.pop())
        )
        object.__setattr__(
            self, "body", nn.Sequential([ResBlock(num_feats=self.embed_dim, key=keys.pop()) for _ in range(2)])
        )
        object.__setattr__(
            self,
            "tail",
            nn.Sequential(
                [
                    UpsamplerBlock(scale_pow_two=1, num_feats=self.embed_dim, key=keys.pop()),
                    Conv2dLayer(in_channels=self.embed_dim, out_channels=self.in_channels, key=keys.pop()),
                ]
            ),
        )
        ...
        object.__setattr__(
            self,
            "HG1_downsample_0",
            DownsampleLayer(in_channels=self.embed_dim, out_channels=self.embed_dim * 2, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "HG1_encoderlayer_1",
        ...
        object.__setattr__(
            self,
            "HG1_downsample_1",
            DownsampleLayer(in_channels=self.embed_dim * 2, out_channels=self.embed_dim * 4, key=keys.pop()),
        )
        ...
        object.__setattr__(
            self,
            "HG1_upsample_0",
            UpsampleLayer(in_channels=self.embed_dim * 4, out_channels=self.embed_dim * 2, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "HG1_decoderlayer_0",
        ...
        object.__setattr__(
            self,
            "HG1_upsample_1",
            UpsampleLayer(in_channels=self.embed_dim * 4, out_channels=self.embed_dim, key=keys.pop()),
        )
        object.__setattr__(
            self,
            "HG1_decoderlayer_1",
