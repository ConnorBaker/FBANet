...
def tensor_divide_burst(tensor_burst, psize, overlap, pad=True):
    ...
    if overlap != 0:
        for batch in range(0, B):
            tensor[batch, :, :, :, :] = tensor_pad(
                tensor_copy[batch, :, :, :, :], (overlap, overlap, overlap, overlap), mode="reflect"
            ).data  # tensor.shape=[T,C,H+2*overlap,W+2*overlap]
    else:
...
def tensor_merge_burst(blocks, tensor, psize, overlap, pad=True):
    ...
    for i in range(h_block):
        for j in range(w_block):
            ...
            tensor_new[:, :, i * psize : end_h, j * psize : end_w] = part[
                :, :, overlap : part.shape[2] - overlap, overlap : part.shape[3] - overlap
            ]
