...
def generate_processed_image_channel4(
    im,
    meta_data,
    return_np=False,
...
def generate_processed_image_channel3(
    im,
    meta_data,
    return_np=False,
    black_level_substracted=True,
    ...
    if not meta_data.get("black_level_subtracted", False) and not black_level_substracted:
        ...
        meta_data["black_level"] = torch.stack(
            (
                meta_data["black_level"][0].float(),
                meta_data["black_level"][1:3].float().mean(),
                meta_data["black_level"][3].float(),
            ),
            dim=0,
        )
    ...
    if not meta_data.get("while_balance_applied", False) and not no_white_balance:
        ...
        meta_data["cam_wb"] = torch.stack(
            (meta_data["cam_wb"][0].float(), meta_data["cam_wb"][1:3].float().mean(), meta_data["cam_wb"][3].float()),
            dim=0,
        )
